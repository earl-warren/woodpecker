...
woodpecker-cli exec --backend-engine=lxc --local=false --log-level=trace --commit-ref=refs/heads/$(git rev-parse --abbrev-ref HEAD) --repo-clone-url=$(pwd) pipeline/backend/lxc/tests/lxc.yml
